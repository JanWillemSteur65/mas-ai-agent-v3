...
export default function ToolsPage({ tenant = "default" }) {
                      ...
                      return (
                        <TableRow {...getRowProps({ row })} key={row.id}>
                          ...
                          <TableCell key={`${row.id}-actions`}>
                            <Stack orientation="horizontal" gap={2}>
                              <Button kind="ghost" size="sm" renderIcon={Edit} disabled={busy || tool?.isBuiltin}
                                onClick={() => openEdit(name)}>
                                Edit
                              </Button>
                              <Button kind="danger--ghost" size="sm" renderIcon={TrashCan} disabled={busy || tool?.isBuiltin}
